...
class FalseGenerator:
    ...
    def corrupt_entity(self, p, e, method = 'disjoint'):
        ...
        if method == 'disjoint':
            ...
            for c in cls:
                for dc in self.disjoint[c]:
                    disjoint_entities |= self.data.subjects(predicate=RDF.type, object=dc)
                    
            if disjoint_entities:
                e = choice(list(disjoint_entities))
            else:
                return self.corrupt_entity(p, e, 'random')
    ...
    def corrupt(self, t = None, method='range', ignore_literals=True, ignore_blank_nodes = True):
        ...
        if 'domain' in method:
            s = self.corrupt_entity(p,s,'domain')
        ...
        if 'range' in method:
            o = self.corrupt_entity(p,o,'range')
        ...
        if 'disjoint' in method:
            if 'range' in method:
                o = self.corrupt_entity(p,o,'disjoint')
            if 'domain' in method:
                s = self.corrupt_entity(p,s,'disjoint')
